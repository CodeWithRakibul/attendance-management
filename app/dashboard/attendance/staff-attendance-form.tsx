...
export function StaffAttendanceForm() {
  ...
  const updateCheckTime = (staffId: string, field: 'checkInTime' | 'checkOutTime', time: string) => {
    setStaff(prev => 
      prev.map(member => 
        member.id === staffId ? { ...member, [field]: time } : member
      )
    );
  };
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <Avatar className="h-10 w-10">
                    <AvatarImage src={member.personal.photoUrl} alt={member.personal.nameEn} />
                    <AvatarFallback>
                      {member.personal.nameEn.split(' ').map(n => n.charAt(0)).join('').toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  ...
                  <div>
                    ...
                    {member.personal.nameBn && (
                      <div className="text-sm text-muted-foreground">{member.personal.nameBn}</div>
                    )}
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex space-x-2">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateCheckTime(member.id, 'checkInTime', e.target.value)}
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateCheckTime(member.id, 'checkOutTime', e.target.value)}
                        className="w-24 h-8 text-xs"
                        disabled={member.status !== 'PRESENT'}
                      />
                    </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      variant={member.status === 'LEAVE' ? 'default' : 'outline'}
                      size="sm"
