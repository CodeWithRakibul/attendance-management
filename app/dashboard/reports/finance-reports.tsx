    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Date Range Filters */}
          {(filters.reportType === 'daily-collection' || filters.reportType === 'collection-vs-expense') && (
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
            ...
            <div className="space-y-6">
              ...
              {filters.reportType === 'collection-vs-expense' && (
                <div className="border rounded-lg p-6">
                  <h4 className="font-medium mb-4">Collection vs Expense Trends</h4>
                  <div className="h-64 bg-muted rounded flex items-center justify-center">
                    <div className="text-center text-muted-foreground">
                      <BarChart3 className="h-12 w-12 mx-auto mb-2" />
                      <p>Collection vs Expense chart would be displayed here</p>
              ...
              {filters.reportType === 'fee-type-wise' && reportData.feeTypeDistribution && (
                <div className="border rounded-lg p-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="h-64 bg-muted rounded flex items-center justify-center">
                      <div className="text-center text-muted-foreground">
                        ...
                        <p>Fee type pie chart would be displayed here</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      {reportData.feeTypeDistribution.map((item: any, index: number) => (
                        <div key={index} className="flex justify-between items-center p-3 border rounded">
                          <span className="font-medium">{item.feeType}</span>
              ...
              {reportData.records && reportData.records.length > 0 && (
                <div className="border rounded-lg overflow-hidden">
                  <div className="bg-muted p-4">
                    <h4 className="font-medium">Sample Financial Records (First 10 records)</h4>
                  </div>
                    ...
                    <table className="w-full">
                      <thead className="bg-muted/50">
                        <tr>
                          ...
                          <th className="text-left p-3 border-b">Collected By</th>
                        </tr>
                      </thead>
                      <tbody>
                        {reportData.records.slice(0, 10).map((record: any, index: number) => (
                          <tr key={index} className="border-b">
                            <td className="p-3">
                              {new Date(record.collectedAt || record.createdAt).toLocaleDateString()}
                            </td>
                            <td className="p-3">
                              {record.student?.personal?.nameEn}
                            </td>
                            <td className="p-3">
                              <Badge variant="outline">{record.feeMaster?.type}</Badge>
                            </td>
                ...
                >
                            ...
                            <td className="p-3">{record.collectedBy || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {filters.reportType === 'class-wise-collection' && reportData.classSummary && (
                <div className="border rounded-lg overflow-hidden">
                  <div className="bg-muted p-4">
                    <h4 className="font-medium">Class-wise Collection Summary</h4>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-muted/50">
                        <tr>
                          ...
                          <th className="text-left p-3 border-b">Total Collection</th>
                          <th className="text-left p-3 border-b">Pending Dues</th>
                          <th className="text-left p-3 border-b">Collection %</th>
                        </tr>
                      </thead>
                      <tbody>
                        {reportData.classSummary.map((summary: any, index: number) => (
                          <tr key={index} className="border-b">
                            ...
                            <td className="p-3">
                              <Badge variant={summary.collectionPercentage >= 80 ? 'default' : 'destructive'}>
              ...
              {filters.reportType === 'fee-defaulters' && reportData.defaulters && (
                <div className="border rounded-lg overflow-hidden">
                    ...
                    <table className="w-full">
                      <thead className="bg-muted/50">
                        <tr>
                          ...
                          <th className="text-left p-3 border-b">Class</th>
                          <th className="text-left p-3 border-b">Due Amount</th>
                          <th className="text-left p-3 border-b">Days Overdue</th>
                          <th className="text-left p-3 border-b">Guardian Contact</th>
                        </tr>
                      </thead>
                      <tbody>
                        {reportData.defaulters.slice(0, 10).map((defaulter: any, index: number) => (
                          <tr key={index} className="border-b">
                            <td className="p-3">{defaulter.studentId}</td>
                            <td className="p-3">{defaulter.personal?.nameEn}</td>
                            <td className="p-3">{defaulter.class?.name}</td>
                            <td className="p-3 text-red-600 font-medium">
                              ৳{defaulter.dueAmount.toLocaleString()}
                            </td>
                            <td className="p-3">
                              <Badge variant="destructive">{defaulter.daysOverdue} days</Badge>
                            </td>
                            <td className="p-3">{defaulter.guardian?.mobile}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  {reportData.defaulters.length > 10 && (
                    <div className="p-3 text-sm text-muted-foreground text-center border-t">
                      ... and {reportData.defaulters.length - 10} more defaulters
                    </div>
                  )}
                </div>
              )}
