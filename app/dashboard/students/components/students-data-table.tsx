...
import { deleteStudentAction, bulkDeleteStudentsAction, updateStudentStatusAction } from '../actions';
...
import { IconDotsVertical, IconEye, IconEdit, IconTrash, IconPhone, IconMail, IconUser, IconCalendar, IconSchool, IconDownload } from '@tabler/icons-react';
...
export function StudentsDataTable({ students, sessions, classes, batches, sections }: StudentsDataTableProps) {
  ...
  const handleStatusUpdate = async (studentId: string, status: 'ACTIVE' | 'INACTIVE' | 'DISABLED') => {
    setIsLoading(true);
    try {
  ...
  const generateCSV = (data: StudentTableData[]) => {
    const headers = ['ID', 'Name (English)', 'Name (Bengali)', 'Class', 'Section', 'Batch', 'Phone', 'Email', 'Status', 'Admission Date'];
  ...
  const columns: ColumnDef<StudentTableData>[] = [
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">{personal?.nameBn || ''}</div>
            </div>
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return formatDate(row.original.admissionDate);
      },
    },
    {
