...
import { getStudents, getSessions, getClasses, getBatches, getSections, getCurrentSession } from '../actions';
...
export async function StudentsPageContent({ searchParams }: StudentsPageContentProps) {
  ...
  const [students, sessions, classes, batches, sections] = await Promise.all([
    getStudents(sessionId, {
      ...
      classId: searchParams.class && searchParams.class !== 'all' ? searchParams.class : undefined,
      status: searchParams.status && searchParams.status !== 'all' ? searchParams.status as any : undefined,
      batchId: searchParams.batch && searchParams.batch !== 'all' ? searchParams.batch : undefined,
    }),
    getSessions(),
  ...
  const filteredStudents = students.filter(student => {
    ...
    if (searchParams.gender && searchParams.gender !== 'all' && personal?.gender !== searchParams.gender) {
      return false;
    }
    if (searchParams.section && searchParams.section !== 'all' && student.section?.name !== searchParams.section) {
      return false;
    }
    return true;
  });
  ...
  const uniqueSections = [...new Set(students.map(s => s.section?.name).filter(Boolean))].sort();
